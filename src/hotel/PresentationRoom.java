...
public class PresentationRoom {
	...
	static public HotelManagement h = new HotelManagement();
	...
	static public CheckIn checkIn = new CheckIn();
	static public Reservation reservation = new Reservation();
	static public InputData inputData = new InputData();
	...
	// 05-12-2023 12:00 이광호 날짜형식 안맞을시 예외처리 추가
	// 05-14-2023 02:08 성지수 예외시 반복 추가, 02:30 날짜 저장 추가
			...
			if (inDay.isAfter(LocalDate.now()) || inDay.isEqual(LocalDate.now())) {
				...
				while (true) {
					...
					if (checkOutDay > 0) {
						...
						h.roomArray[i[0]][i[1]].setCheckInDay(inDay);
						h.roomArray[i[0]][i[1]].setCheckOutDay(outDay);
	...
	// 05-12-2023 01:12 성지수 예약 안한사람 체크인중(체크인날짜를 now로)
	public int checkInNowDate(int num) {
		System.out.println("숙박 일수 입력");
		while (true) {
			int checkInDayNum = inputData.userInputNumber();
			if (checkInDayNum > 0) {
				LocalDate inDay = LocalDate.now();
				System.out.println("체크인 날짜: " + inDay);
				LocalDate outDay = LocalDate.now().plusDays(checkInDayNum);
				System.out.println("체크아웃 날짜: " + outDay);
				int[] i = h.divideRoomNum(num);
				h.roomArray[i[0]][i[1]].setCheckInDay(inDay);
				h.roomArray[i[0]][i[1]].setCheckOutDay(outDay);
				return checkInDayNum;
			} else {
				System.out.println("1 이상의 정수를 입력하세요.");
			}
		}
	...
	public void payRoom(Person person, int day, int num) {
		if (customerManagement.identifyVIP2(person)) {
			if (num % 2 != 0) {
				printPayPriceVIP(day, Price.SINGLE_PRICE, h.roomType(num), num);
			} else {
				printPayPriceVIP(day, Price.DOUBLE_PRICE, h.roomType(num), num);
			}
		} else {
			if (num % 2 != 0) {
				printPayPrice(day, Price.SINGLE_PRICE, h.roomType(num), num);
			} else {
				printPayPrice(day, Price.DOUBLE_PRICE, h.roomType(num), num);
			}
		}
	}
