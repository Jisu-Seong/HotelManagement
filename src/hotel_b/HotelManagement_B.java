...
public class HotelManagement_B {
	static public Room[][] roomArray_B = new Room[3][10];
	public static Price_B price = new Price_B();
	...
	public void ShowTotalRooms() {
		System.out.print("빈 객실 : O / 예약된 객실 : △  / 사용중인 객실 : X / 청소 중인 객실 : ●");
		System.out.print("\t1 - 5호실 : 싱글 / 6 - 10호실 : 더블\n");
		divisionLine();
		for (int i = 0; i < roomsLength(); i++) {
			for (int j = 0; j < roomsLength(i); j++) {
	...
	public int showRoom(Predicate<Room> roomState) {
		...
		for (int i = 0; i < roomsLength(); i++) {
			for (int j = 0; j < roomsLength(i); j++) {
				if (roomState.test(roomArray_B[i][j])) {
	...
	public void reservationIndex(Person person, int num) {
		int[] i = divideRoomNum(num);
		roomArray_B[i[0]][i[1]].setPerson(person);
	...
	public void checkOutIndex(int num) {
		...
		roomArray_B[i[0]][i[1]].setPerson(null);
	...
	// 05-14-2023 01:20 성지수 방의 청소상태 랜덤
	...
	public void printCheckDay(int i, int j) {
		System.out.println("체크인 날짜: " + roomArray_B[i][j].getCheckInDay());
		System.out.println("체크아웃 날짜: " + roomArray_B[i][j].getCheckOutDay());
	}
}